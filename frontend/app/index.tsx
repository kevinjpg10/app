...
export default function Index() {
    ...
    <View style={styles.formGroup}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoryScroll}>
        {CATEGORIES.map((cat) => (
          <TouchableOpacity
            key={cat}
            style={[styles.categoryChip, editForm.category === cat && { backgroundColor: CATEGORY_COLORS[cat] }]}
            onPress={() => setEditForm({ ...editForm, category: cat })}
          >
            <Text style={[styles.categoryChipText, editForm.category === cat && { color: '#fff' }]}>{cat}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
    </View>
  ...
  const renderFormFields = () => (
    <>
      <View style={styles.formGroup}>
        ...
        <TextInput style={styles.formInput} value={editForm.establishment_name}
          onChangeText={(text) => setEditForm({ ...editForm, establishment_name: text })}
          placeholder="Nombre del establecimiento" placeholderTextColor="#666" />
      </View>
      {renderCategorySelector()}
      <View style={styles.formGroup}>
        <Text style={styles.formLabel}>CIF</Text>
        <TextInput style={styles.formInput} value={editForm.cif}
          onChangeText={(text) => setEditForm({ ...editForm, cif: text })}
          placeholder="B12345678" placeholderTextColor="#666" autoCapitalize="characters" />
      </View>
      <View style={styles.formGroup}>
        <Text style={styles.formLabel}>Dirección</Text>
        <TextInput style={styles.formInput} value={editForm.address}
          onChangeText={(text) => setEditForm({ ...editForm, address: text })}
          placeholder="Calle, número, ciudad" placeholderTextColor="#666" />
      </View>
      <View style={styles.formGroup}>
        <Text style={styles.formLabel}>Teléfono</Text>
        <TextInput style={styles.formInput} value={editForm.phone}
          onChangeText={(text) => setEditForm({ ...editForm, phone: text })}
          placeholder="912345678" placeholderTextColor="#666" keyboardType="phone-pad" />
      </View>
      <View style={styles.formGroup}>
        <Text style={styles.formLabel}>Total (€) *</Text>
        <TextInput style={styles.formInput} value={editForm.total}
          onChangeText={(text) => setEditForm({ ...editForm, total: text.replace(',', '.') })}
          placeholder="0.00" placeholderTextColor="#666" keyboardType="decimal-pad" />
      </View>
      <View style={styles.formGroup}>
        <Text style={styles.formLabel}>Fecha</Text>
        <TextInput style={styles.formInput} value={editForm.date}
      ...
      <View style={styles.formGroup}>
        ...
        <View style={styles.paymentToggle}>
          <TouchableOpacity style={[styles.paymentOption, editForm.payment_method === 'efectivo' && styles.cashActive]}
          ...
          <TouchableOpacity style={[styles.paymentOption, editForm.payment_method === 'tarjeta' && styles.cardActive]}
                ...
                {chartData.map((item, index) => (
                  <View key={index} style={styles.barRow}>
                    ...
                    <View style={styles.barContainer}>
                      <View style={[styles.bar, { backgroundColor: item.color, width: `${(item.value / maxValue) * 100}%` }]} />
                    </View>
                    <Text style={styles.barValue}>{item.text}</Text>
                  </View>
                ))}
              </View>
            ) : (
          ...
          <ScrollView style={styles.detailContainer}>
            ...
            {isEditing ? (
              ...
              <View style={styles.detailCard}>
                <View style={styles.detailRow}><Text style={styles.detailLabel}>Establecimiento</Text><Text style={styles.detailValue}>{selectedExpense.establishment_name || 'No disponible'}</Text></View>
                <View style={styles.detailRow}><Text style={styles.detailLabel}>Categoría</Text><View style={[styles.categoryBadge, { backgroundColor: CATEGORY_COLORS[selectedExpense.category] || '#888' }]}><Text style={styles.categoryBadgeText}>{selectedExpense.category || 'Varios'}</Text></View></View>
                <View style={styles.detailRow}><Text style={styles.detailLabel}>CIF</Text><Text style={styles.detailValue}>{selectedExpense.cif || 'No disponible'}</Text></View>
                <View style={styles.detailRow}><Text style={styles.detailLabel}>Dirección</Text><Text style={styles.detailValue}>{selectedExpense.address || 'No disponible'}</Text></View>
                <View style={styles.detailRow}><Text style={styles.detailLabel}>Teléfono</Text><Text style={styles.detailValue}>{selectedExpense.phone || 'No disponible'}</Text></View>
                <View style={styles.detailRow}><Text style={styles.detailLabel}>Total</Text><Text style={styles.detailValueBold}>€{selectedExpense.total.toFixed(2)}</Text></View>
                <View style={styles.detailRow}><Text style={styles.detailLabel}>Fecha</Text><Text style={styles.detailValue}>{selectedExpense.date || 'No disponible'}</Text></View>
                <View style={styles.detailRow}>
                  <Text style={styles.detailLabel}>Método de pago</Text>
                  <TouchableOpacity onPress={() => togglePaymentMethod(selectedExpense.id)}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {renderStatsModal()}
      <View style={styles.header}>
        <View><Text style={styles.headerTitle}>Efrain</Text><Text style={styles.headerSubtitle}>Asistente de Gastos</Text></View>
        <View style={styles.headerActions}>
          <TouchableOpacity onPress={() => setShowStats(true)} style={styles.statsButton}><Ionicons name="pie-chart" size={22} color="#4A90D9" /></TouchableOpacity>
          <TouchableOpacity onPress={deleteAllExpenses} style={styles.deleteAllButton}><Ionicons name="trash-outline" size={22} color="#ff4444" /></TouchableOpacity>
          <TouchableOpacity onPress={downloadExcel} style={styles.downloadButton}><Ionicons name="download-outline" size={24} color="#fff" /></TouchableOpacity>
        </View>
      </View>
      {summary && (
        <View style={styles.summaryContainer}>
          <View style={styles.summaryCard}><Ionicons name="receipt-outline" size={24} color="#4A90D9" /><Text style={styles.summaryAmount}>€{summary.total_amount.toFixed(2)}</Text><Text style={styles.summaryLabel}>{summary.total_expenses} gastos</Text></View>
          <View style={styles.summaryCard}><Ionicons name="card-outline" size={24} color="#9B59B6" /><Text style={styles.summaryAmount}>€{summary.card_payments.total.toFixed(2)}</Text><Text style={styles.summaryLabel}>{summary.card_payments.count} tarjeta</Text></View>
          <View style={styles.summaryCard}><Ionicons name="cash-outline" size={24} color="#27AE60" /><Text style={styles.summaryAmount}>€{summary.cash_payments.total.toFixed(2)}</Text><Text style={styles.summaryLabel}>{summary.cash_payments.count} efectivo</Text></View>
        </View>
      )}
      <View style={styles.tabsContainer}>
        <TouchableOpacity style={[styles.tab, activeTab === 'all' && styles.activeTab]} onPress={() => setActiveTab('all')}><Text style={[styles.tabText, activeTab === 'all' && styles.activeTabText]}>Todos</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.tab, activeTab === 'card' && styles.activeTab]} onPress={() => setActiveTab('card')}><Ionicons name="card" size={16} color={activeTab === 'card' ? '#fff' : '#888'} /><Text style={[styles.tabText, activeTab === 'card' && styles.activeTabText]}>Tarjeta</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.tab, activeTab === 'cash' && styles.activeTab]} onPress={() => setActiveTab('cash')}><Ionicons name="cash" size={16} color={activeTab === 'cash' ? '#fff' : '#888'} /><Text style={[styles.tabText, activeTab === 'cash' && styles.activeTabText]}>Efectivo</Text></TouchableOpacity>
      </View>
      {loading ? (
        <View style={styles.loadingContainer}><ActivityIndicator size="large" color="#4A90D9" /><Text style={styles.loadingText}>Cargando gastos...</Text></View>
      ) : (
        <ScrollView style={styles.expensesList} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#4A90D9" />}>
          {filteredExpenses.length === 0 ? (
            <View style={styles.emptyContainer}><Ionicons name="receipt-outline" size={64} color="#444" /><Text style={styles.emptyText}>No hay gastos registrados</Text><Text style={styles.emptySubtext}>Sube un ticket o añade manualmente</Text></View>
          ) : (
            filteredExpenses.map((expense) => (
              <View key={expense.id} style={styles.expenseCard}>
                <TouchableOpacity style={styles.expenseLeft} onPress={() => setSelectedExpense(expense)}>
                  <TouchableOpacity onPress={() => togglePaymentMethod(expense.id)}
      ...
      {uploading ? (
        <View style={styles.uploadingContainer}><ActivityIndicator size="large" color="#fff" /><Text style={styles.uploadingText}>Procesando ticket con IA...</Text></View>
      ) : (
        <TouchableOpacity style={styles.fab} onPress={showAddOptions}><Ionicons name="add" size={32} color="#fff" /></TouchableOpacity>
      )}
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  barChartContainer: { paddingVertical: 10 },
  totalCenter: { alignItems: 'center', marginBottom: 20, paddingVertical: 20, borderBottomWidth: 1, borderBottomColor: '#2a2a2a' },
  barRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  barColorDot: { width: 12, height: 12, borderRadius: 6, marginRight: 8 },
  barLabel: { color: '#888', fontSize: 12, width: 80 },
  barContainer: { flex: 1, height: 24, backgroundColor: '#2a2a2a', borderRadius: 12, marginHorizontal: 8, overflow: 'hidden' },
  bar: { height: '100%', borderRadius: 12 },
  barValue: { color: '#fff', fontSize: 12, fontWeight: '600', width: 45, textAlign: 'right' },
});
